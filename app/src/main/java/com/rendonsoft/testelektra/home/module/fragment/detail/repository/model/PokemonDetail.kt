package com.rendonsoft.testelektra.home.module.fragment.detail.repository.model
...
fun PokemonDetail.toView(name : String) : PokemonDetailView{
    val eggGroupBuffer = StringBuffer()
    if(this.egg_groups.isNotEmpty()) {
        this.egg_groups.forEachIndexed { index, eggGroup ->
            if (index < this.egg_groups.size - 1) eggGroupBuffer.append("${eggGroup.name}, ") else eggGroupBuffer.append(
                eggGroup.name
            )
        }
    }
    return PokemonDetailView(
        name = name,
        baseHappiness = this.base_happiness.toString(),
        color = this.color.name,
        captureRate = this.capture_rate.toString(),
        eggGroup = eggGroupBuffer.toString(),
        evolutionUrl = this.evolution_chain.url
    )
}